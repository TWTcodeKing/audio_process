...
class GRDB(nn.Module):
    def __init__(self,C_In,C_Out,Ksize,Padding,Stride):
        ...
        self.ResConv = nn.Conv1d(
            kernel_size=(1,1),
            in_channels=C_In,
            out_channels=C_Out
        )
        self.Gradient_OP = nn.Conv1d(
            kernel_size= (1,Ksize),
            in_channels = C_In,
            out_channels = C_In,
            stride = (1,1),
    ...
    def forward(self,x):
        Res = self.Gradient_OP(x)
        Res = self.ResConv(Res)
        Direct1 = self.Conv1(x) + x
        Direct2 = self.Conv2(Direct1) + Direct1 +x
        Direct2 = self.Conv3(Direct2)
        return Direct2 + Res
        pass


class Group_block(nn.Module):
    def __init__(self,Max_channels,In_channels,Res_layers,Fusion_layers,Ksize,Padding,Stride):
        super(Group_block,self).__init__()
        self.GRDBs_Source = nn.ModuleList()
        self.GRDBs_Mixture = nn.ModuleList()
        self.Fusion_Convs = nn.ModuleList()
        self.Res_layers = Res_layers
        self.Fusion_layers = Fusion_layers
        # Max_channels == 16*pow(2,Res_layers)
        out_channels = Max_channels//pow(2,Res_layers)*2
        init_channels = In_channels
        for i in range(Res_layers):
            self.GRDBs_Mixture.append(GRDB(
                    C_In= init_channels,
                    C_Out = out_channels,
                    Ksize= Ksize,
                    Padding = Padding,
    ...
    def forward(self,source,mixture):
        for i in range(self.Res_layers):
            source = self.GRDBs_Source[i](source)
            mixture = self.GRDBs_Mixture[i](mixture)
        Fusioner = torch.cat([source,mixture],dim=1)
        for i in range(self.Fusion_layers):
            Fusioner = self.Fusion_Convs[i](Fusioner)
        return Fusioner
        pass
