...
class SiSnrPitWrapper(nn.Module):
    """
    ...
    def __init__(self, base_loss):
        super(SiSnrPitWrapper, self).__init__()
...
class StoiPitWrapper(nn.Module):
    """
    Permutation Invariant Wrapper to allow Permutation Invariant Training
    (PIT) with existing losses.
    ...
    def __init__(self, base_loss):
        super(StoiPitWrapper, self).__init__()
        self.base_loss = base_loss
    ...
    def _opt_perm_loss(self,preds,targets):

        n_sources = preds.size(-1)
        loss_mat = list()
        for i in range(preds.size(1)):
            y_pred = preds[:, i].unsqueeze(0)
            for j in range(targets.size(1)):
                y_true = targets[:,j].unsqueeze(0)
                loss = self.base_loss(y_pred,y_true,torch.ones(1))
                loss_mat.append(loss.data)

        loss_mat = np.asarray(loss_mat).reshape((1,n_sources,n_sources))
        loss_mat = torch.from_numpy(loss_mat).cuda()
        assert (
                len(loss_mat.shape) >= 2
        ), "Base loss should not perform any reduction operation"
        mean_over = [x for x in range(len(loss_mat.shape))]
...
def get_si_snr_with_pitwrapper(source, estimate_source):
    ...
    pit_si_snr = SiSnrPitWrapper(cal_si_snr)
    loss, perms = pit_si_snr(source, estimate_source)

    return loss

def get_stoi_with_pitwrapper(source, estimate_source):
    pit_stoi = StoiPitWrapper(stoi_loss)
    loss, perms = pit_stoi(source, estimate_source)
    return loss
...
def DCT_Loss(est,ground_truth):
    return torch.nn.MSELoss(est,ground_truth)


if __name__ == "__main__":
    source = torch.rand(1,17428,2).cuda()
    est_source = torch.rand(1,17428,2).cuda()
    #loss1 = get_si_snr_with_pitwrapper(source,est_source)
    loss2 = get_stoi_with_pitwrapper(source,est_source)
    print(loss2)